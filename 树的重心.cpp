...
bool get_child(int *current_visited_idx, int root, int *child)
{
    int idx = *current_visited_idx;
    while (idx != -1) {
...
int dfs(int root)
{
    ...
    int child;
    int current_visited_idx = head[root];
    while (get_child(&current_visited_idx, root, &child)) {
